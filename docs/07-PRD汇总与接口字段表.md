...
| 编号 | 模块 | 需求 | 优先级 |
|---|---|---|---|
| FR-01 | 开户 | 业务员录入企业档案；**企业级确权（法人意愿认证/电子章/盖章授权书）为开户硬门槛**，未确权经办人提货受限 | P0 |
...
### 5.1.1 企业级确权 `POST /enterprise/{id}/authorize`
> 开户硬门槛：确立企业级意思表示，未完成前经办人不可正式提货（见 `02` 文档 2.6 节）。
**入参**
| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| method | E | 是 | `legal_face`法人意愿认证 / `e_seal`企业电子章 / `sealed_doc`盖章授权书 |
| legalPersonName | S | 否 | 法人姓名（method=legal_face，比对工商登记） |
| legalPersonIdCard | S | 否 | 法人身份证号（加密，method=legal_face） |
| faceToken | S | 否 | 法人人脸核身令牌 |
| agreementVersion | S | 是 | 签署的服务协议版本 |
| authorizedOperatorIds | A | 是 | 授权的经办人ID列表 |
| sealFileUrl | S | 否 | 盖章文件（method=sealed_doc/e_seal） |

**出参**
| 字段 | 类型 | 说明 |
|---|---|---|
| authStatus | E | `authorized` 已确权 |
...
### 5.2 提货单在线确认 `POST /pickuporder/{id}/confirm`
> **单据角色**：留货单仅为通知（不作确认依据）；核心确认单据为**提货单**（载明品名/规格/数量/单价/金额）。本接口为经办人在线确认提货单，一次性完成"认可货物价款 + 授权提货 + 收货确认"并生成提货码（与 5.3 可合并为一次确认）。
**入参**
| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| operatorId | S | 是 | 经办人ID |
...
| 枚举 | 取值 |
|---|---|
| 经办人账号状态 | 待实名 / 已实名待确权 / 已实名授权 / 已停用 / 已失效 |
| 企业确权状态 | 未确权 / 法人意愿认证 / 电子章 / 盖章授权书 / 已确权 |
